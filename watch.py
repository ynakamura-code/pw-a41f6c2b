...
CSS = """:root{--bg:#f6f7f9;--card:#fff;--line:#e3e6ea;--tx:#1c1e21;--sub:#6b7280;
...
.prow2{margin-top:8px;padding-top:8px;border-top:1px solid var(--line);font-size:13px;color:var(--sub)}
.plink{display:inline-block;margin:3px 6px 3px 0;padding:3px 10px;background:#f2f4f7;
 border:1px solid var(--line);border-radius:999px;color:var(--tx);text-decoration:none;font-size:13px}
.plink:hover{background:#e8edf5;border-color:#c7d2e0}
.plink.on{background:#1c1e21;color:#fff;border-color:#1c1e21}
.srclist{list-style:none;padding:0;margin:0}
.srclist li{background:#fff;border:1px solid var(--line);border-radius:8px;padding:8px 12px;margin-bottom:6px}
.srclist a{color:var(--tx)}
.evday{margin:0 0 18px}
...
def run(only=None, shots=True, send=True):
    ...
    cutoff = (now_dt - timedelta(days=KEEP_DAYS)).strftime("%Y-%m-%d")
    history = [h for h in history if h.get("checked_at", "")[:10] >= cutoff][:3000]
...
def platform_list():
    """platforms/*.json から (id, 名前) を並び順で返す。"""
    out = []
    for fn in sorted(os.listdir(PLATFORM_DIR)):
        if not fn.endswith(".json"):
            continue
        with open(os.path.join(PLATFORM_DIR, fn), encoding="utf-8") as f:
            p = json.load(f)
        out.append((p["id"], p["name"], p.get("order", 100), p["sources"]))
    out.sort(key=lambda x: (x[2], x[1]))
    return [(x[0], x[1]) for x in out]


def platform_sources(pid):
    for fn in sorted(os.listdir(PLATFORM_DIR)):
        if not fn.endswith(".json"):
            continue
        with open(os.path.join(PLATFORM_DIR, fn), encoding="utf-8") as f:
...
def build_picker(days, cur):
    ...
    opts = "".join("<option value='%s.html'%s>%s%s</option>"
                   % (esc(d), " selected" if d == cur else "", esc(d),
                      "（最新）" if d == days[0] else "")
                   for d in days)
    links = " ".join("<a class='plink' href='p-%s.html'>%s</a>" % (esc(pid), esc(name))
                     for pid, name in platform_list())
    return ("<div class='picker'>"
            "<div class='prow'>見る日付 <select onchange=\"location.href=this.value\">%s</select>"
            "<span class='pn'>%d日分を保存しています（最大%d日）</span></div>"
            "<div class='prow2'>プラットフォーム別にまとめて見る： %s</div>"
            "</div>" % (opts, len(days), KEEP_DAYS, links))
...
def write_platform_pages(history, results, days, now):
    """プラットフォームごとに、保存している期間の変更をまとめた1ページを作る。"""
    latest = {}
    for r in results:
        latest[(r["platform"], r["source"])] = r
    for pid, name in platform_list():
        plat = platform_sources(pid) or {"sources": []}
        evs = [h for h in history if h.get("platform") == pid]

        # いま見ているページの一覧
        rows = []
        for src in plat["sources"]:
            r = latest.get((pid, src["id"]), {})
            url = r.get("url") or src.get("url", "")
            label = esc(src["label"])
            kind = esc(KIND.get(src["kind"], (src["kind"],))[0])
            if r.get("error"):
                state = "<span style='color:#a52a1f'>取得できませんでした</span>"
            elif r.get("changed"):
                state = "<b>今日 変更あり</b>"
            else:
                state = esc(r.get("note") or "変更なし")
            title = ("<a href='%s' target='_blank'>%s ↗</a>" % (esc(url), label)) if url else label
            rows.append("<li><span class='tag'>%s</span>%s<span class='sub'>%s</span></li>"
                        % (kind, title, state))

        # 変更の履歴（日付ごと）
        byday = {}
        for h in evs:
            byday.setdefault(h["checked_at"][:10], []).append(h)
        blocks = []
        for d in sorted(byday, reverse=True):
            ...
            for h in byday[d]:
                h = dict(h)
                h.setdefault("anchor", "")
                mark = "🔴 " if impact_of(h) else "👀 "
                link = ("<a href='%s' target='_blank'>%s ↗</a>"
                        % (esc(h.get("url", "")), esc(h["label"]))) if h.get("url") else esc(h["label"])
                cards.append("<div class='ev'><h3>%s%s</h3>%s</div>" % (mark, link, card_body(h)))
            page_link = "<a class='back' href='%s.html'>この日のレポートを見る →</a>" % esc(d)
            blocks.append("<div class='evday'><b>%s</b> %s%s</div>"
                          % (esc(d), page_link, "".join(cards)))

        links = " ".join("<a class='plink%s' href='p-%s.html'>%s</a>"
                         % (" on" if q == pid else "", esc(q), esc(nm))
                         for q, nm in platform_list())
        nav = ("<div class='picker'><div class='prow'>"
               "<a class='back' href='index.html'>← 最新のレポートに戻る</a>"
               "<span class='pn'>%d日分を保存しています（最大%d日）</span></div>"
               "<div class='prow2'>プラットフォーム別： %s</div></div>" % (len(days), KEEP_DAYS, links))

        doc = """<!doctype html><html lang="ja"><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>%s の変更履歴｜プラットフォーム変更ウォッチ</title>
<style>__CSS__</style>
<div class="wrap">
<h1>%s の変更履歴</h1>
...
<details><summary>すべて見る — クリックで開きます</summary><div class="hbox">%s</div></details>
        ...
        with open(os.path.join(SITE, "p-%s.html" % pid), "w", encoding="utf-8") as f:
            f.write(doc)
...
<details><summary>すべて見る（%d 件）— クリックで開きます</summary><div class="hbox">%s</div></details>
    ...
    with open(REPORT, "w", encoding="utf-8") as f:
        f.write(doc)
    prune_site()
    kept = site_days()[:KEEP_DAYS]
    refresh_pickers(kept)
    write_platform_pages(history, results, kept, now)
