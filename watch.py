...
def write_report(results, history, now):
    ...
    for day in sorted(hist, reverse=True):
        hrows.append("<div class='hday'><b>%s</b><ul>" % esc(day))
        for h in hist[day]:
    ...
    days = sorted(set(site_days() + [day]), reverse=True)
    opts = "".join("<option value='%s.html'%s>%s%s</option>"
                   % (esc(d), " selected" if d == day else "", esc(d),
                      "（最新）" if d == days[0] else "")
                   for d in days)
    picker = ("<div class='picker'>見る日付 <select onchange=\"location.href=this.value\">%s</select>"
              "<span class='pn'>過去%d日分を保存しています</span></div>" % (opts, len(days)))
